...
const QHash<QString,QString> MoresamplerConfig::entryHelps{
    {"output-sampling-rate",QT_TRANSLATE_NOOP("", "输出的 .wav 文件的采样频率")},
    {"output-bit-depth",QT_TRANSLATE_NOOP("", "输出的 .wav 文件的位深")},
    {"resampler-compatibility",QT_TRANSLATE_NOOP("", "开启时, 在resampler模式下生成 .wav 文件 （用于与其他wavtool兼容）。此选项将减慢合成。 ")},
    {"synthesis-utau-style-normalization",QT_TRANSLATE_NOOP("", "对每个音符应用自适应增益，使得当音量为100％时，合成波形的峰值达到最大幅度的一半。full: 对浊音和清音均进行增益； voiced: 只增益浊音；off: 不改变音量。")},
    {"synthesis-loudness-preservation",QT_TRANSLATE_NOOP("", "基于心理声学响度测量，在修改后使可感知响度保持。")},
    {"synthesis-duration-extension-method",QT_TRANSLATE_NOOP("", "决定Moresampler如何使音符时间变长。auto: 根据源时长和目标时长自动决定是使用拉伸方式还是循环方式。可使用 e 和 Me flag 来覆盖这一行为。")},
    {"multithread-synthesis",QT_TRANSLATE_NOOP("", "on: wavtool模式下的合成将多线程运行（将会更快）。full: resampler模式也将在多线程运行。“多线程”意味着将多个线程包装于一个进程中，与同一时间运行多个Moresampler的“多进程”有着本质不同。注意，该特性不被 moresampler-legacy.exe支持。")},
    {"auto-update-llsm-mrq",QT_TRANSLATE_NOOP("", "检查.wav 文件与对应.llsm 文件和mrq数据项的最后修改时间。如果.wav文件比.llsm要新，则进行重分析。若.wav文件也比mrq数据项要新，则在重分析以生成.llsm文件前进行音高重估计。此特性或许对声库开发者有帮助。 ")},
    {"dump-log-file",QT_TRANSLATE_NOOP("", "在指定文件路径中输出Debug信息。")},
    {"analysis-f0-range-from-path",QT_TRANSLATE_NOOP("", "从文件夹名推测音高。例如，如果给定的样本位于一个名为“C_D4”的文件夹下，Moresampler将在接近D4的范围（294 Hz左右）内进行音高分析。")},
    {"analysis-biased-f0-estimation",QT_TRANSLATE_NOOP("", "在接合音高与浊音行为估计期间增大浊音化概率，随后进行浊音音高修正；旨在提高特异度和降低灵敏度；适用于嘈杂/粗糙的语音，但会降低干净/流畅语音的质量。 ")},
    {"analysis-f0-min",QT_TRANSLATE_NOOP("", "音高的下界(以Hz为单位)。")},
    {"analysis-f0-max",QT_TRANSLATE_NOOP("", "音高的上界(以Hz为单位)。")},
    {"load-frq",QT_TRANSLATE_NOOP("", "strict : 当desc.mrq不可用时，从.frq 文件读取音高数据。on : 使用 .frq 文件的音高数据来修正Moresampler自己对音高的猜测。不管该选项是否开启，结果都会保存至desc.mrq。注意：除非被仔细修正，.frq文件提供的音高数据通常无法足够可靠地使Moresampler运作。")},
    {"analysis-anti-distortion",QT_TRANSLATE_NOOP("", "on : Moresampler将自动修复由噪声失真或低音量（量化误差）引起的分析错误。这些错误可能造成音高变换后的“尖锐”与“粗糙”的声音。然而此特性（理论上）可能导致轻微模糊。")},
    {"analysis-noise-reduction",QT_TRANSLATE_NOOP("", "在从.wav文件分析LLSM时自动降噪。更适用于长录音。")},
    {"analysis-suppress-subharmonics",QT_TRANSLATE_NOOP("", "在分析时自动移除输入中存在的次谐波。对尖亮的声音可能有帮助，但会导致呼吸音的质量降低。")},
};
